...
import org.trainopia.pms.features.user.dao.UserDAOImpl;
...
public class UserServiceImpl implements UserService {
    ...
    public User create(CreateUserDTO createUserDTO) {
        User existingUser = userDAO.findByEmailOrUserName(createUserDTO.email(), createUserDTO.userName()).orElse(null);
        if (existingUser != null) {
            throw new AppException("create", this.getClass().getSimpleName(), "User Exist", CommonError.USER_EXISTS, HttpStatus.BAD_REQUEST);
        }
        User newUser = new User(createUserDTO.firstName(), createUserDTO.lastName(), UserRole.VOLUNTEER);
        String encryptedPassword = bCryptPasswordEncoder.encode(createUserDTO.password());
        UserLoginData userLoginData= new UserLoginData(createUserDTO.userName(),createUserDTO.email(),encryptedPassword);
